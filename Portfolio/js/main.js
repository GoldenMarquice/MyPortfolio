      ...
      projectsListElement.innerHTML += `
        ...
        <div class="project-card">
            <a href="${projects[i].repository}" target="blank">
                <img src="${projects[i].img}" alt="" class="card-img">
...
fetch("./data/experiences.json")
  ...
  .then((json) => {
    ...
    for (let i = 0; i < experiencesSize; i++) {
      ...
      const technologiesSkillsElement = document.getElementById(
        `technology-skill-${i}`
      );
      const technologiesSkillsSize = experiences[i].technologies.length;

      for (let j = 0; j < technologiesSkillsSize; j++) {
        technologiesSkillsElement.innerHTML += `
                    <label> ${experiences[i].technologies[j]} </label>
                `;
      }
    }
  });
